...
if len(indices):
    targets = ['?', 'da', 'nu']
    index = indices[0] # row with ?
else:
    targets = ['da', 'nu']
    index = len(X)-1 # last row
...
print("Unknown:"); print(df.loc[[index]])
...
# Random Forest
model = RandomForestClassifier(random_state=42, max_depth=3, n_estimators=20)
model.fit(X1, Y1)
score1 = model.score(X1, Y1)
score2 = model.score(X2, Y2)
y_pred = model.predict(x_new)
